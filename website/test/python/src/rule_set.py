...
class RuleSet:
    ...
    def _compare(self,val1,val2):
        ...
        try : 
            ...
            if val2[0:2] == "r:" :
                isRegex2 = True
                val2 = val2[2:]
                escape2 = self.escape(self,val2)
            if not isRegex:
                regex1 = self.toregex(self,val1)
    ...
    def _contains2(self,t1,t2):
        ...
        for node2 in t2:
            check = False
            for node1 in t1:
                if self._compare(self,node1.value,node2.value) == Relation.EQUALITY.value and (node2.value != '' and node1.value !='\\n') and ( node1.value !='...' or node2.value !='...' and not(self.isMeta(self,node1.value) or self.isMeta(self,node2.value))): 
                    check = self._contains2(self,node1.children,node2.children)
            if check == False:
                return False      
        return True
    ...
    def _contains1(self,tree1,tree2):
        for node2 in tree2:
            ...
            for node1 in range(len(tree1)):
                if self._compare(self,tree1[node1].value,node2.value) == Relation.EQUALITY.value and (node2.value != '' and tree1[node1].value !='\\n') and ( tree1[node1].value !='...' or node2.value !='...' and not(self.isMeta(self,tree1[node1].value) or self.isMeta(self,node2.value))):
                    if self._contains2(self,tree1[node1].children,node2.children) == True:
                    ...
                    for item in tree1[node1].children : 
                        child.append(item)     
            if(child != []):
                return(self._contains1(self,child,tree2))
            else :return False

    def _equals(self,tree1,tree2):
        if (self._compare(self,tree1.value,tree2.value) == Relation.EQUALITY.value or self._compare(self,tree2.value,tree1.value) == Relation.EQUALITY.value) and (tree2.value != '' and tree1.value !='\\n') and ( tree1.value !='...' or tree2.value !='...' and not(self.isMeta(self,tree1.value) or self.isMeta(self,tree2.value))):
            if(len(tree1.children) != len(tree2.children)):
                return False
            for i in range(len(tree1.children)):
                if self._equals(self,tree1.children[i],tree2.children[i]) == False : 
                    return False
            return True
        else : 
            return False
    ...
    def _overlap2(self,sub1,sub2):
        ...
        for node2 in sub2.children:
            for node1 in sub1.children:
                if self._compare(self,node1.value,node2.value) == Relation.EQUALITY.value and (node2.value!= '' and node1.value!='\\n') and ( node1.value!='...' or node2.value!='...' and not(self.isMeta(self,node1.value) or self.isMeta(self,node2.value))):
                    tmpval = self._overlap2(self,node1,node2)
                    if tmpval != None : 
    ...
    def _overlap(self,tree1,tree2):
        ...
        for n2 in tree2:
            ...
            for node1 in tree1:
                if self._compare(self,node1.value,n2.value) == Relation.EQUALITY.value and (n2.value != '' and node1.value !='\\n') and ( node1.value !='...' or n2.value !='...' and not(self.isMeta(self,node1.value) or self.isMeta(self,n2.value))):
                    if n2.value == "OR" or n2.value == "AND" or n2.value == "NOT": 
                        myl = []
                        for elem1 in node1.children:
                            for elem2 in n2.children:
                                if self._compare(self,elem1.value,elem2.value) == Relation.EQUALITY.value and (elem2.value != '' and elem2.value !="\\n"):
                                    if( elem1.value !='...' or elem2.value !='...' and not(self.isMeta(self,elem1.value) or self.isMeta(self,elem2.value))) : 
                        ...
                        if myl != [] :
                            ...
                            for sub in myl : 
                                tmpVal = self._overlap2(self,sub[0],sub[1])
    ...
    def _interval(self,val1, val2,n1,n2):
        ...
        if (self._equals(self,t1,t2)):
            return Relation.EQUALITY.value
        elif(self._contains1(self,[t1],[t2])):
            print(n2)
            print(n1)
            print('Connection.INCLUSION')
            return Relation.INCLUSION_IJ.value
        elif(self._contains1(self,[t2],[t1])):
            print(n1)
            print(n2)
            print('Connection.INCLUSION')
            return Relation.INCLUSION_JI.value
        elif( self._overlap(self,[t1],[t2]) != [] ):
            v = self._overlap(self,[t1],[t2])
            subtr = rebuild(v[0])
            clean_tr = self.clean(self,subtr)
            if clean_tr =="": 
                return Relation.DIFFERENCE.value 
            print(n1)
            print(n2)
